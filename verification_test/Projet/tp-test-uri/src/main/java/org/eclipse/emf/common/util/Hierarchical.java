...
final class Hierarchical extends URI {
    ...
    public String path() {
        ...
        if (hasAbsolutePath()) result.append(URI_CONST.SEGMENT_SEPARATOR);

        String[] segments = this.segments;
        for (int i = 0, len = segments.length; i < len; i++) {
            if (i != 0) result.append(URI_CONST.SEGMENT_SEPARATOR);
    ...
    public String devicePath() {
        ...
        if (hasAuthority()) {
            if (!isArchive()) result.append(URI_CONST.AUTHORITY_SEPARATOR);
            result.append(authority);

            if (hasDevice()) result.append(URI_CONST.SEGMENT_SEPARATOR);
        }
    ...
    public URI appendQuery(String query) {
        return URI_CONST.POOL.intern(false, URIPool.URIComponentsAccessUnit.VALIDATE_QUERY, true, scheme, authority, device, hasAbsolutePath(), segments, query);
    }
    ...
    public URI trimQuery() {
        if (query == null) {
            ...
            return URI_CONST.POOL.intern(false, URIPool.URIComponentsAccessUnit.VALIDATE_NONE, true, scheme, authority, device, hasAbsolutePath(), segments, null);
        }
    }
    ...
    public URI resolve(URI base, boolean preserveRootParents) {
        ...
        if (authority == null) {
            ...
            if (device == null) {
                ...
                if (hasEmptyPath() && query == null) {
                    ...
                    newSegments = newAbsolutePath ? mergePath(base, preserveRootParents) : URI_CONST.NO_SEGMENTS;
                }
        ...
        return URI_CONST.POOL.intern(false, URIPool.URIComponentsAccessUnit.VALIDATE_NONE, true, base.scheme(), newAuthority, newDevice, newAbsolutePath, newSegments, newQuery);
    }
    ...
    protected String[] mergePath(URI base, boolean preserveRootParents) {
        ...
        if (sp > 0) {
            if (segmentCount == 0) {
                ...
                if (segment == URI_CONST.SEGMENT_EMPTY || segment == URI_CONST.SEGMENT_PARENT || segment == URI_CONST.SEGMENT_SELF) {
                    stack[sp++] = URI_CONST.SEGMENT_EMPTY;
                }
            }
        }
    ...
    protected static int accumulate(String[] stack, int sp, String segment, boolean preserveRootParents) {
        if (URI_CONST.SEGMENT_PARENT == segment) {
            if (sp == 0) {
                ...
                if (URI_CONST.SEGMENT_PARENT == stack[sp - 1]) stack[sp++] = segment;
                else sp--;
            }
        } else if (URI_CONST.SEGMENT_EMPTY != segment && URI_CONST.SEGMENT_SELF != segment) {
    ...
    public URI deresolve(URI base, boolean preserveRootParents, boolean anyRelPath, boolean shorterRelPath) {
        ...
        return URI_CONST.POOL.intern(false, URIPool.URIComponentsAccessUnit.VALIDATE_NONE, true, null, newAuthority, newDevice, newAbsolutePath, newSegments, newQuery);
    }
    ...
    protected boolean hasCollapsableSegments(boolean preserveRootParents) {
        ...
        for (int i = 0, len = segments.length; i < len; i++) {
            ...
            if (i < len - 1 && URI_CONST.SEGMENT_EMPTY == segment ||
                    ...
                    URI_CONST.SEGMENT_PARENT == segment && (!preserveRootParents || i != 0 && URI_CONST.SEGMENT_PARENT != segments[i - 1])) {
                return true;
            }
        }
        return false;
    ...
    protected String[] findRelativePath(URI base, boolean preserveRootParents) {
        ...
        if (length == 0) {
            if (query == null) return URI_CONST.ONE_SELF_SEGMENT;
    ...
    protected boolean matches(String string) {
        ...
        if (!isRelative()) {
            ...
            index += scheme.length();
            if (index >= length || string.charAt(index) != URI_CONST.SCHEME_SEPARATOR) {
                return false;
            }
            ++index;
        }
        ...
        if (hasDevice()) {
            if (index >= length || string.charAt(index) != URI_CONST.SEGMENT_SEPARATOR) {
                return false;
            }
            ++index;
        ...
        if (hasQuery()) {
            if (index >= length || string.charAt(index) != URI_CONST.QUERY_SEPARATOR) {
                return false;
            }
            ++index;
    ...
    protected boolean matches(String base, String path) {
        ...
        for (int i = 1, len = segments.length, index = 1; i < len; i++) {
            if (i != 1) {
                if (index >= length || path.charAt(index) != URI_CONST.SEGMENT_SEPARATOR) {
                    return false;
                }
                ++index;
            }
    ...
    public URI appendSegment(String segment) {
        ...
        if (isEmptySegment && segments.length == 0) {
            ...
            newSegments = URI_CONST.NO_SEGMENTS;
        } else if (device == null && segments.length == 0 && !isEmptySegment && segment.charAt(segment.length() - 1) == URI_CONST.DEVICE_IDENTIFIER) {
        ...
        return URI_CONST.POOL.intern(false, segments.length, true, scheme, authority, newDevice, newAbsolutePath, newSegments, query);
    }
    ...
    public URI appendSegments(String[] segments) {
        ...
        return URI_CONST.POOL.intern(false, this.segments.length, true, scheme, authority, device, newAbsolutePath, newSegments, query);
    }
    ...
    public URI trimSegments(int i) {
        ...
        return URI_CONST.POOL.intern(false, URIPool.URIComponentsAccessUnit.VALIDATE_NONE, true, scheme, authority, device, hasAbsolutePath(), newSegments, query);
    }
    ...
    public URI appendFileExtension(String fileExtension) {
        ...
        return URI_CONST.POOL.intern(false, len, true, scheme, authority, device, hasAbsolutePath(), newSegments, query);
    }
    ...
    public URI replacePrefix(URI oldPrefix, URI newPrefix) {
        ...
        if (oldPrefixSegmentCount == 0) {
            ...
            if (i == segmentsLength - 1 && segments[i] == URI_CONST.SEGMENT_EMPTY) {
                return newPrefix;
            } else {
        ...
        // no validation needed since all components are from existing URIs
        return URI_CONST.POOL.intern(false, URIPool.URIComponentsAccessUnit.VALIDATE_NONE, true, newPrefix.scheme(), newPrefix.authority(), newPrefix.device(), newPrefix.hasAbsolutePath(), mergedSegments, query);
    }
}
